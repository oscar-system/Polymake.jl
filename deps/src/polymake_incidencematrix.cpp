...
void polymake_module_add_incidencematrix(jlcxx::Module& polymake)
{
    ...
    .add_type<jlcxx::Parametric<jlcxx::TypeVar<1>>, jlcxx::ParameterList<bool,int>>(
        "pm_IncidenceMatrix", jlcxx::julia_type("AbstractSparseMatrix", "SparseArrays"))
        ...
        wrapped.method("_row", [](WrappedT& M, int64_t i) { return pm::Set<int>(M.row(i - 1)); });
        wrapped.method("cols", &WrappedT::cols);
        wrapped.method("_col", [](WrappedT& M, int64_t i) { return pm::Set<int>(M.col(i - 1)); });
