@testset "bigobj" begin
    ...
    @testset "template parameters" begin
        ...
        @test (@pm polytope.Polytope{Polymake.Rational}(input_dict_int)) isa BigObject
        @test (@pm polytope.Polytope{QuadraticExtension}(input_dict_int)) isa BigObject
        @test (@pm polytope.Polytope{QuadraticExtension{Polymake.Rational}}(input_dict_int)) isa BigObject
        ...
        @test (@pm tropical.Polytope{Max}(input_dict_int)) isa BigObject
        @test (@pm tropical.Polytope{Max, Polymake.Rational}(input_dict_int)) isa BigObject
        ...
        P = @pm polytope.Polytope{Polymake.Rational}(POINTS=[1 0.5 0; 1 0 1])
        @test P.VERTICES isa Polymake.Matrix{Polymake.Rational}
    end
    ...
    @testset "tab-completion" begin
        ...
        @test Base.propertynames(test_polytope) isa Polymake.Vector{Symbol}
        ...
        @test Base.propertynames(g) isa Polymake.Vector{Symbol}
    end
    ...
    @testset "polymake MILP" begin
        ...
        @test (@pm polytope.MixedPolymake.IntegerLinearProgram( LINEAR_OBJECTIVE = obj, INTEGER_VARIABLES = intvar)) isa Polymake.BigObject

        pmintvar = Polymake.Set(intvar)

        @test (@pm polytope.MixedPolymake.IntegerLinearProgram( LINEAR_OBJECTIVE = obj, INTEGER_VARIABLES = pmintvar)) isa Polymake.BigObject

        p.MILP = @pm polytope.MixedPolymake.IntegerLinearProgram( LINEAR_OBJECTIVE = obj, INTEGER_VARIABLES = intvar)
