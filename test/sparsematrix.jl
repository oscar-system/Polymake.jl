...
@testset "Polymake.SparseMatrix" begin
        ...
        for T in IntTypes #TODO Polymake.Integer
            ...
            for ElType in [Polymake.Integer, Polymake.Rational, Float64]
                for m in [jl_m, jl_m//T(1), jl_m/T(1)]
                    ...
                    @test convert(Polymake.Matrix{T}, M) isa Polymake.Matrix{T}
                    @test jl_m == convert(Polymake.Matrix{T}, M)
                end
                for s in [jl_s, jl_s//T(1), jl_s/T(1)] #TODO
                    ...
                    # @test convert(SparsePolymake.Arrays.Polymake.SparseMatrixCSC{T,}, S) isa SparsePolymake.Arrays.Polymake.SparseMatrixCSC{T}
                    # @test jl_s == convert(SparsePolymake.Arrays.Polymake.SparseMatrixCSC{T}, S)
                end
            end
            ...
            for m in [jl_m, jl_m//T(1), jl_m/T(1), jl_s, jl_s//T(1), jl_s/T(1)]
                ...
                @test Polymake.Matrix{Float64}(M) isa Polymake.Matrix{Float64}
            ...
            let W = Polymake.SparseMatrix{Polymake.Rational}(jl_m)
                for T in [Base.Rational{I} for I in IntTypes]
                    @test convert(Polymake.Matrix{T}, W) isa Polymake.Matrix{T}
                    @test jl_m == convert(Polymake.Matrix{T}, W)
                end
            end
            ...
            let W = Polymake.SparseMatrix{Polymake.Rational}(jl_s)
                for T in [Base.Rational{I} for I in IntTypes]
                    @test convert(Polymake.Matrix{T}, W) isa Polymake.Matrix{T}
                    @test jl_s == convert(Polymake.Matrix{T}, W)
                end
            end

            let U = Polymake.SparseMatrix{Float64}(jl_m)
                for T in FloatTypes
                    @test convert(Polymake.Matrix{T}, U) isa Polymake.Matrix{T}
                    @test jl_m == convert(Polymake.Matrix{T}, U)
                end
            end
            ...
            let U = Polymake.SparseMatrix{Float64}(jl_s)
                for T in FloatTypes
                    @test convert(Polymake.Matrix{T}, U) isa Polymake.Matrix{T}
                    @test jl_s == convert(Polymake.Matrix{T}, U)
                end
            end
        end
    end
    ...
    @testset "Low-level operations" begin
        @testset "Polymake.SparseMatrix{Int64}" begin
            ...
            for T in [IntTypes; Polymake.Integer]
                ...
                V[2, 1] = T(10)
                @test V[2, 1] == 10
                @test string(V) == "pm::Polymake.SparseMatrix<int, pm::NonSymmetric>\n5 2 3\n10 5 6\n"
            end

            @test string(Polymake.SparseMatrix{Int64}(jl_s)) == "pm::Polymake.SparseMatrix<int, pm::NonSymmetric>\n(3)\n(3) (1 1)\n"
        end
        ...
        @testset "Polymake.SparseMatrix{Polymake.Integer}" begin
            ...
            for T in [IntTypes; Polymake.Integer]
                ...
                V[2, 1] = T(10)
                @test V[2, 1] == 10
                @test string(V) == "pm::Polymake.SparseMatrix<pm::Polymake.Integer, pm::NonSymmetric>\n5 2 3\n10 5 6\n"
            end

            @test string(Polymake.SparseMatrix{Polymake.Integer}(jl_s)) == "pm::Polymake.SparseMatrix<pm::Polymake.Integer, pm::NonSymmetric>\n(3)\n(3) (1 1)\n"
        end
        ...
        @testset "Polymake.SparseMatrix{Polymake.Rational}" begin
            ...
            for T in [IntTypes; Polymake.Integer]
                ...
                V[2] = T(10)//T(3)
                @test V[2] == 10//3
                @test string(V) == "pm::Polymake.SparseMatrix<pm::Polymake.Rational, pm::NonSymmetric>\n5/3 2 3\n10/3 5 6\n"
            end

            @test string(Polymake.SparseMatrix{Polymake.Rational}(jl_s)) == "pm::Polymake.SparseMatrix<pm::Polymake.Rational, pm::NonSymmetric>\n(3)\n(3) (1 1)\n"
        end
        ...
        @testset "Polymake.SparseMatrix{Float64}" begin
            ...
            for T in [IntTypes; Polymake.Integer]
                ...
                @test string(V) == "pm::Polymake.SparseMatrix<double, pm::NonSymmetric>\n1.66667 2 3\n3.33333 5 6\n"
            end

            @test string(Polymake.SparseMatrix{Float64}(jl_s)) == "pm::Polymake.SparseMatrix<double, pm::NonSymmetric>\n(3)\n(3) (1 1)\n"
        end
    ...
    @testset "Arithmetic" begin
        V = Polymake.SparseMatrix{Polymake.Integer}(jl_m)
        @test float.(V) isa Polymake.Polymake.SparseMatrixAllocated{Float64}
        # @test V[1, :] isa Polymake.SparsePolymake.VectorAllocated{Polymake.Integer}
        # @test float.(V)[1, :] isa SparsePolymake.Vector{Float64}

        @test similar(V, Float64) isa Polymake.Polymake.SparseMatrixAllocated{Float64}
        # @test similar(V, Float64, 10) isa Polymake.SparsePolymake.VectorAllocated{Float64}
        @test similar(V, Float64, 10, 10) isa Polymake.Polymake.SparseMatrixAllocated{Float64}
        ...
        @test -X isa Polymake.Polymake.SparseMatrixAllocated{Int64}
        @test -X == -jl_m

        @test -V isa Polymake.Polymake.SparseMatrixAllocated{Polymake.Integer}
        @test -V == -jl_m

        @test -W isa Polymake.Polymake.SparseMatrixAllocated{Polymake.Rational}
        @test -W == -jl_w

        @test -U isa Polymake.Polymake.SparseMatrixAllocated{Float64}
        @test -U == -jl_u

        int_scalar_types = [IntTypes; Polymake.Integer]
        rational_scalar_types = [[Base.Rational{T} for T in IntTypes]; Polymake.Rational]

        @test 2X isa Polymake.SparseMatrix{Polymake.Integer}
        @test Int64(2)X isa Polymake.SparseMatrix{Int64}
