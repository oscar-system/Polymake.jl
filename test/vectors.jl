@testset "Polymake.Vector" begin
    ...
    @testset "Constructors/Converts" begin
        ...
        for T in [IntTypes; Polymake.Integer]
            @test Polymake.Vector(T.(jl_v)) isa Polymake.Vector{T == Int64 ? Int64 : Polymake.Integer}
            ...
            for ElType in [Polymake.Integer, Polymake.Rational, Float64]
                for v in [jl_v, jl_v//T(1), jl_v/T(1)]
                    ...
                    V = Polymake.Vector(v)
                    @test convert(Polymake.Vector{T}, V) isa Polymake.Vector{T}
                    @test jl_v == convert(Polymake.Vector{T}, V)
                end
            end
            ...
            for v in [jl_v, jl_v//T(1), jl_v/T(1)]
                ...
                @test Polymake.Vector{Float64}(V) isa Polymake.Vector{Float64}
            ...
            let W = Polymake.Vector{Polymake.Rational}(jl_v)
                for T in [Base.Rational{I} for I in IntTypes]
                    @test convert(Polymake.Vector{T}, W) isa Polymake.Vector{T}
                    @test jl_v == convert(Polymake.Vector{T}, W)
                end
            end

            let U = Polymake.Vector{Float64}(jl_v)
                for T in FloatTypes
                    @test convert(Polymake.Vector{T}, U) isa Polymake.Vector{T}
                    @test jl_v == convert(Polymake.Vector{T}, U)
                end
            end
        end
    end
    ...
    @testset "Low-level operations" begin
        ...
        @testset "Polymake.Vector{Int64}" begin
            ...
            for T in [IntTypes; Polymake.Integer]
                ...
                V[2] = T(10)
                @test V[2] == 10
                @test string(V) == "pm::Polymake.Vector<int>\n5 10 3"
            end
        end
        ...
        @testset "Polymake.Vector{Polymake.Integer}" begin
            ...
            for T in [IntTypes; Polymake.Integer]
                ...
                V[2] = T(10)
                @test V[2] == 10
                @test string(V) == "pm::Polymake.Vector<pm::Polymake.Integer>\n5 10 3"
            end
        end
        ...
        @testset "Polymake.Vector{Polymake.Rational}" begin
            ...
            for T in [IntTypes; Polymake.Integer]
                ...
                V[2] = T(10)//T(3)
                @test V[2] == 10//3
                @test string(V) == "pm::Polymake.Vector<pm::Polymake.Rational>\n5/3 10/3 3"
            end
        end
        ...
        @testset "Polymake.Vector{Float64}" begin
            ...
            for T in [IntTypes; Polymake.Integer]
                ...
                V[2] = T(10)//T(3)
                @test V[2] ≈ 10/3
                @test string(V) == "pm::Polymake.Vector<double>\n1.66667 3.33333 3"
            end
        end
    ...
    @testset "Arithmetic" begin
        ...
        @test 2X isa Polymake.Vector{Polymake.Integer}
        @test Int64(2)X isa Polymake.Vector{Int64}
